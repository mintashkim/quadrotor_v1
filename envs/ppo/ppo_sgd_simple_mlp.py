...
def traj_segment_generator(pi, gamma, env, horizon, stochastic):
    ...
    while True:
        ...
        if t > 0 and t % horizon == 0:
            yield {"ob_vf" : obs_vf, "ob_pol": obs_pol, "rew" : rews, "vpred" : vpreds, "new" : news,
...
def learn(env, policy_fn, *,
    ...
    pi = policy_fn("pi", ob_space_vf, ob_space_pol, ac_space) # Construct network for new policy
    oldpi = policy_fn("oldpi", ob_space_vf, ob_space_pol, ac_space) # Network for old policy
    ...
    lossandgrad = U.function([ob_vf, ob_pol, ac, atarg, ret, lrmult], losses + [U.flatgrad(total_loss, var_list)])
    adam = MpiAdam(var_list, epsilon=adam_epsilon)

    assign_old_eq_new = U.function([],[], updates=[tf.assign(oldv, newv)
        for (oldv, newv) in zipsame(oldpi.get_variables(), pi.get_variables())])
    compute_losses = U.function([ob_vf, ob_pol, ac, atarg, ret, lrmult], [losses, clip_frac])
    ...
    rewbuffer_pos = deque(maxlen=100)
    rewbuffer_vel = deque(maxlen=100)
    rewbuffer_input = deque(maxlen=100)
    ...
    while True and max_iters != 1 :
        ...
        # todo
        ob_vf, ob_pol, ac, atarg, tdlamret = seg["ob_vf"], seg["ob_pol"], seg["ac"], seg["adv"], seg["tdlamret"]
        vpredbefore = seg["vpred"] # predicted value function before udpate
        atarg = (atarg - atarg.mean()) / atarg.std() # standardized advantage function estimate
        d = Dataset(dict(ob_vf=ob_vf, ob_pol=ob_pol, ac=ac, atarg=atarg, vtarg=tdlamret), deterministic=pi.recurrent)
        ...
        logger.log(fmt_row(18, loss_names))
        # Here we do a bunch of optimization epochs over the data
        for _ in range(optim_epochs):
            losses = [] # list of tuples, each of which gives the loss for a minibatch
            for batch in d.iterate_once(optim_batchsize):
                *newlosses, g = lossandgrad(batch["ob_vf"], batch["ob_pol"], batch["ac"], batch["atarg"], batch["vtarg"], cur_lrmult)
                adam.update(g, optim_stepsize * cur_lrmult)
                losses.append(newlosses)
            logger.log(fmt_row(18, np.mean(losses, axis=0)))

        logger.log("Evaluating losses...")
        losses = []
        clip_fracs = []
        for batch in d.iterate_once(optim_batchsize):
            newlosses, newclipfrac = compute_losses(batch["ob_vf"], batch["ob_pol"], batch["ac"], batch["atarg"], batch["vtarg"], cur_lrmult)
            losses.append(newlosses)
            clip_fracs.append(newclipfrac)
        meanlosses,_,_ = mpi_moments(losses, axis=0)
        meanclipfracs, _, _ = mpi_moments(clip_fracs, axis=0)

        logger.log(fmt_row(18, meanlosses))
        ...
        lrlocal = (seg["ep_lens"], seg["ep_rets"],
                  ...
                  #  seg["ep_rets_detail"][:, 13],
                  #  seg["ep_rets_detail"][:, 14],
                  #  seg["ep_rets_detail"][:, 15]
        listoflrpairs = MPI.COMM_WORLD.allgather(lrlocal) # list of tuples
        lens, rews, rews_pos, rews_vel, rews_input, rews_delta_acs\
            = map(flatten_lists, zip(*listoflrpairs))

        lenbuffer.extend(lens)
        rewbuffer.extend(rews)
        rewbuffer_pos.extend(rews_pos)
        rewbuffer_vel.extend(rews_vel)
        rewbuffer_input.extend(rews_input)
        ...
        logger.record_tabular("rewards/EpRewPos", np.mean(rewbuffer_pos))
        logger.record_tabular("rewards/EpRewVel", np.mean(rewbuffer_vel))
        logger.record_tabular("rewards/EpRewInput", np.mean(rewbuffer_input))
        logger.record_tabular("rewards/EpRewDeltaAcs", np.mean(rewbuffer_delta_acs))
