...
sys.path.append('/Users/mintaekim/Desktop/HRL/Flappy/Integrated/Flappy_Integrated/quadrotor_v1/envs')
import numpy as np
...
class QuadrotorEnv(MujocoEnv, utils.EzPickle):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.n_action           = 4  # NOTE: change to the number of action
        ...
        self.pos_lb = np.array([-20, -20, 0.1]) # fight space dimensions: xyz
        self.pos_ub = np.array([20, 20, 20])
        self.vel_lb = np.array([-3, -3, -3])
        self.vel_ub = np.array([3, 3, 3])

        self.action_lower_bounds = np.array([0,0,0,0])
        self.action_upper_bounds = np.array([5,5,5,5])
